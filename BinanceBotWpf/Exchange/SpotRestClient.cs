...
{
    public class SpotRestClient : IDisposable, IBinanceRestClient, Services.IBinanceClient
    {
        ...
        private readonly ConcurrentDictionary<string, decimal> _minNotionalCache = new ();
        private readonly ConcurrentDictionary<string, (decimal Price, DateTime Expiry)> _priceCache = new ();
        private decimal _cachedBalance;
        private DateTime _balanceCacheExpiry = DateTime.MinValue;
        private readonly SemaphoreSlim _balanceLock = new (1, 1);
        private readonly SemaphoreSlim _dustLock = new (1, 1);
        ...
        private readonly SemaphoreSlim _requestQueueLock = new (1, 1);

        public SpotRestClient(string apiKey, string apiSecret, bool useTestnet = false)
        {
            _apiKey = apiKey;
        ...
        // SendWithRetryAsync — retry с использованием Polly для transient errors
            ...
            var policy = Policy<HttpResponseMessage>
                ...
                .WaitAndRetryAsync (
                    ...
                    onRetry: (outcome, delay, retryAttempt, context) =>
                    {
                        ...
                        Log ($"Повтор {retryAttempt}/{maxRetries} через {delay.TotalMilliseconds:F0}ms ({msg})");
                    });
            ...
            return await policy.ExecuteAsync (async (ctx) =>
                {
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            Log ($"⚠️ Timestamp -1021. Пересинхронизация и повтор");
        ...
        public async Task<List<BinanceKline>> GetKlinesAsync(string symbol, string interval, int limit, DateTime? endTime)
        {
            ...
            {
                Log ($"❌ GetKlinesAsync {symbol}: {ex.Message}");
                return new List<BinanceKline> ();
            }
            finally
            {
        ...
        public async Task<decimal> GetAccountBalanceAsync(string asset)
        {
            if (asset == "USDC")
            {
                lock (_balanceLock)
                {
                    if (_cachedBalance > 0 && DateTime.UtcNow < _balanceCacheExpiry)
                        ...
                        if (assetStr == asset)
                        {
                            if (decimal.TryParse (balance["free"]?.ToString (), NumberStyles.Any, CultureInfo.InvariantCulture, out decimal free))
                            {
                                if (asset == "USDC")
                                {
                                    ...
                                    {
                                        _cachedBalance = free;
                                        _balanceCacheExpiry = DateTime.UtcNow.AddSeconds (BalanceCacheSeconds);
                                    }
                                }
                                return free;
                            }
        ...
        public async Task<JArray> GetFlexibleEarnBalanceAsync()
        {
            try
            {
                ...
                while (hasMore)
                {
                    ...
                    {
                        Log ($"⚠️ GetFlexibleEarnBalanceAsync: превышен лимит страниц ({maxPages})");
                        break;
                    }
        ...
        private async Task ThrottleAsync(int estimatedWeight = 1)
        {
            ...
            try
            {
                ...
                if (_currentWeight + estimatedWeight > _maxWeightPerMinute)
                {
                    ...
                    await _throttleLock.WaitAsync ();
                    // Only reset if no other thread already did it
